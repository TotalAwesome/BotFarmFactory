...
class BotFarmer(BaseFarmer):
    ...
    def play_game(self):
        if not GAME_TOGGLE_ON:
            ...
            for playpass in range(self.play_passes or 0):
                if self.play_passes <= GAME_BANK: 
                    self.log(MSG_GAME_KEEP.format(GAME_BANK-self.play_passes))
                    return
                elif playpass == GAME_ROUNDS:
                    return                    
                self.log(MSG_BEGIN_GAME.format(self.play_passes, bank=GAME_BANK, play=(GAME_ROUNDS-playpass)))
