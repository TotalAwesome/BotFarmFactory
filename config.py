...
DEBUG = True
RETRY_ATTEMPTS = 3
SLEEP_BETWEEN_START = [20, 180]

ENABLED_BOTS = [
    # Пустой список = все боты включены
    # иначе будут работать только те, что в этом списке

    # Пример заполнения списка, если нужно оставить только нужных ботов
    # (# слева убираем, запятые обязательны, названия должны соответствовать названиям из папки bots)
    # 'blum',
    # 'hamster_kombat',
]
